...
file_name = "/content/netflix-app/netflix_titles.csv"

# Commented out IPython magic to ensure Python compatibility.
# %cd /content/netflix-app
...
x = pd.DataFrame({'rating': rating_counts.index, 'counts': rating_counts.values})
pieChart = px.pie(
    x,
    values='counts',
    names='rating',
...
# Clean country column (some titles have multiple countries listed)
df['country'] = df['country'].fillna('Unknown')  # Replace NaN values
df['country'] = df['country'].apply(lambda x: x.split(',')[0])  # Keep only the first country
...
# Choropleth Map
st.subheader("Choropleth Map: Netflix Titles by Country")
...
!streamlit run streamlit_netflix_dashboard.py &>/dev/null &

!pip install streamlit pyngrok pandas plotly



!ngrok config add-authtoken 2sMJkqZg1eZrNtd80QReNLIKEZZ_6wVCSxsBRpGdyHsWAWHwP
